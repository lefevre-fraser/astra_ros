		...
		PUBLIC template<typename FrameType> void frameToPointCloud(astra::Frame& frame) {
			...
			if (std::is_base_of<astra::DepthFrame, FrameType>()) {
				...
				if (framePtr->is_valid()) {
					SET_LWH_RESIZE_2(*framePtr, this->depthCloud)
					for (uint h = 0; h < height; ++h) {
						for (uint w = 0; w < width; ++w) {
...
point.a = (aColor);
				...
				if (framePtr->is_valid()) {
					SET_LWH_RESIZE_2(*framePtr, this->colorCloud)
					for (uint h = 0; h < height; ++h) {
						for (uint w = 0; w < width; ++w) {
							uint index = (h*width)+w;
				...
				if (framePtr->is_valid()) {
					SET_LWH_RESIZE_2(*framePtr, this->ir16Cloud)
					for (uint h = 0; h < height; ++h) {
						for (uint w = 0; w < width; ++w) {
							uint index = (h*width)+w;
				...
				if (framePtr->is_valid()) {
					SET_LWH_RESIZE_2(*framePtr, this->irRgbCloud)
					for (uint h = 0; h < height; ++h) {
						for (uint w = 0; w < width; ++w) {
							uint index = (h*width)+w;
